# PR003946-34 - Test Specification Analysis
...
## Description
Current authenticated user. Purpose: implement GET /me. Auth: Requires bearerAuth. Parameters: USERiD  Request body: None. Responses: 200: User profile (User). Returns info about the logged in user200,404, 401 Admin can get anybody (parents can get info about the children) If not a parent can only get info about yourself.  Check token-find role-if parent do lookup to make sure they are in the family.  Lookup on profile. Transmit id of user if Admin they get anything If zookeeper not asking about yourself, you get a 401, If not in the family you get a 401
