...
These templates are recommended to be deployed in the `us-west-2` region.
...
| `backend`            | [Template](backend.yml) &#124; [Doc](backend.md)                       | Core serverless backend for CMZ Chatbots: Lambda, API Gateway, Cognito, DynamoDB, and optional frontend hosting. |
...
**Reminder:**  
When deploying or updating any stack, **use the same tags (`cmz:application`, `cmz:team`, etc.) that were used to deploy `account-guardrails.yml`**. This ensures you have permissions to update or manage the resources created by these templates.

---

**Tip:**
Keep documentation up to date as templates evolve. Use clear, concise language and provide examples and illustrations if possible.
